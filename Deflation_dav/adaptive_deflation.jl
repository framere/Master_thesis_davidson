        ...
        while converged_in_block < Nlow && length(Eigenvalues) < Nlow * blocks
            ...
            for i = 1:Nlow
                ...
                if rnorm < thresh
                    ...
                    if proj_norm < 1e-3  # adaptive duplicate check
                        println("converged eigenvalue ", Σ[i], " with residual norm ", rnorm)
                        push!(Ritz_vecs, X[:, i])
...
function define_matrix(system::String)
    ...
    Nlow = 29   # we are interested in the first Nlow eigenvalues
